 ...
 *
 *      Version:        0.5.1
 *      Edited by:      Buðra Yener Þahinoðlu
 *      Last Update on: Apr 14, 2021
 *      Description: 	getHead() and getTail() functions have been added to access the head and tail nodes outside of the class.
 *
 *      Version: 		0.5
...
public:
	...
	T front() const;
	T back() const;
    LLNode<T>* getHead() const;
    LLNode<T>* getTail() const;
	template <typename S>
...
template <typename T>
LLNode<T>* LinkedList<T>::getHead() const{
	assert(head != NULL&&"Linked List has no elements");
	return head;
}

template <typename T>
LLNode<T>* LinkedList<T>::getTail() const{
	assert(tail != NULL&&"Linked List has no elements");
	return tail;
}
